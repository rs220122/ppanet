...
COCO_PRETRAIN_CKPT=ppa/logs/coco_thing/2019-11-28/model.ckpt-150000
TRAIN_LOGDIR_AUG=ppa/logs/${DATASET}/${NOW_DATE}_trainaug
DATASET_DIR=./dataset/VOCdevkit/tfrecord

# train 60K to model on pascal voc trainaug set.
mkdir -p ${TRAIN_LOGDIR_AUG}
cp ppa/scripts/pascal/train.sh ${TRAIN_LOGDIR_AUG}'/train_copy.sh'
python train.py \
                --output_stride=16 \
                --crop_size=512,512 \
                --batch_size=8 \
...
TRAIN_LOGDIR=ppa/logs/${DATASET}/${NOW_DATE}_train
