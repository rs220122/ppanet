...
# Description: Utility functions related to preprocessing inputs.
#
# ===============================================
...
def flip_dim(tensor_list, prob=0.5, dim=1):
    ...
    Note that tf.random_flip_left_right_and tf.random_flip_up_down isn't used so
    ...
    Returns:
        ...
        `Tensor` at the endo whose value is `True` if the inputs were flipped and
        `False` otherwise.

    Raises:
        ValueError: If dim is negative or greater than the dimention of a `Tensor`.
    """
...
def pad_to_bounding_box(image, offset_height, offset_width, target_height,
    ...
    Works like tf.image.pad_to_bounding_box, except it can pad the image with
    any given arbitrary pad value and also handle images whose sizes are not known
    during graph construction.

    Args:
        image: 3-D tensor with shape [height, width, channels]
        offset_height: Number of rows of zeros to add on top.
        offset_width : Number of columns of zeros to addn on left.
    ...
    with tf.name_scope(None, 'pad_to_bounding_box', [image]):
        ...
        if (original_dtype != tf.float32) and (original_dtype != tf.float64):
            # If image dtype is not float, we convert it to int 32 to avoid overflow.
...
def random_crop(image_list, crop_height, crop_width):
    ...
    The function applies the same crop to each image in the list.
    This can be effectively applies when there are multiple image inputs of the
    same dimension such as:
    ...
    for i in range(len(image_list)):
        ...
        rank_assert = tf.Assert(
            ...
            ['Wrong rank for tensor %s [expected] [actial]',
...
def resolve_shape(tensor, rank=None, scope=None):
    ...
    Use as much as possible the shape component already known during graph
...
def resize_to_range(image,
    ...
    Args:
        ...
        label: (optinal) A 3D tensor of shape [height, width, channels] (default)
          or [channels, height, width] when label_layout_is_chw=True.
        min_size: (scalar) desired size of the smaller image side.
        max_size: (scalar) maximum allowed size of the larger image side.
                  Note that the output dimension is no larger than max_size and may be slightly
                  smaller than min_size when factor is not None.
        factor: Make output size multiple of factor plus one.
        align_corners: If true, the label has shape [channel, height, width].
    ...
    with tf.name_scope(scope, 'resize_to_range', [image]):
        ...
        [original_height, original_width, _] = resolve_shape(image, rank=3)
        original_height = tf.cast(original_height, tf.float32)
        original_width = tf.cast(original_width, tf.float32)
        original_min_size = tf.minimum(original_height, original_width)

        # Calculate the larger of the possible sizes
        large_scale_factor = min_size / original_min_size
        large_height = tf.to_int32(tf.ceil(original_height * large_scale_factor))
        large_width = tf.to_int32(tf.ceil(original_width * large_scale_factor))
        ...
        if max_size is not None:
            ...
            # is too big.
            original_max_size = tf.maximum(original_height, original_width)
            small_scale_factor = max_size / original_max_size
            small_height = tf.to_int32(tf.ceil(original_height * small_scale_factor))
            small_width = tf.to_int32(tf.ceil(original_width * small_scale_factor))
