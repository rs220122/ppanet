...
# Description: input preprocess helper functions.
#
# ===============================================
...
# user packages
from lib.utils import preprocess_utils
...
def preprocess_image_and_label(image,
    ...
    Args:
        ...
        max_resize_value: Maximum scale factor value.
        resize_factor:　Resized dimensions are multiple of factor plus one.
        min_scale_factor: Minimum scale factor value.
        max_scale_factor: Maximum scale factor value.
        scale_factor_step_size: The step size from min scale factor to max scale
                                factor. The input is randomly scaled based on the
                                value of (min_scale_factor, max_scale_factor, scale_factor_step_size).
        ignore_label: The label value which will be ignored for training and evaluation.
        is_training : If the preprocessing is u    return original_image, processed_image, label
sed for training or not.
    ...
    # Pad image and label to have dimensions greater than [crop_height, crop_width].
    ...
    if is_training:
        ...
        processed_image, label, _ = preprocess_utils.flip_dim(
            [processed_image, label], _PROB_OF_FLIP, 1)
