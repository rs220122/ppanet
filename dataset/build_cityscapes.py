...
This dataset is expected to have the following directory structure:
    ...
    + dataset
      - build_cityscapes.py (current working directory).
      - build_data.py
      + cityscapes
...
tf.app.flags.DEFINE_string('annotation_dir',
                           os.path.join('cityscapes', 'gtFine'),
                           'cityscapes annotation directory.')

# direcotry contains images.
tf.app.flags.DEFINE_string('image_dir',
                           os.path.join('cityscapes', 'leftImg8bit'),
                           'cityscapes image directory.')

# directory in which is saved files converted to TFRecord.
tf.app.flags.DEFINE_string('output_dir',
                           os.path.join('cityscapes', 'tfrecord'),
                           'Path to save coverted SSTable of TensorFlow examples.')
